...
export async function generateResultPrompt(
  ...
  return withRetry(async () => {
    ...
    if (isCreamStyle) {
      ...
      - Furniture: An iconic ivory-colored Togo-style tufted sofa (very important). A minimalist low-profile white coffee table. 
    ...
    const response = await callGeminiApi("models/gemini-3-flash-preview", {
      parts: [
        {
          text: `基于以下分析和SaaS平台提供的内容，请生成一段用于高端AI生图的高级英文提示词（Prompt）。
...
生图核心要求（必须严格遵守以避免被当作“简单纸片贴图放大/缩小”）：
1. 真实3D空间铺设（CRITICAL 3D TRIAL FITTING）：地毯不能像是后期悬空P图。地毯必须是100%铺设（laid perfectly flat）在房間地板上，具有准确的三维倾斜、透视缩进和物理重力感（natural weight）。
2. 家具压盖与接触阴影（FURNITURE OVERLAPPING & OC_SHADOWS）：地毯边缘和表面必须与现场家具（例如沙发的桌脚、茶几的底座、休闲椅腿、花盆）深度融合并产生前后压盖关系。这些家具角部必须“站在/站在压在（stand right on top of）”地毯织物上，且在衔接处产生极度逼真柔和的深色接触阴影（Ambient Occlusion Shading/contact shadows）。
3. 主题与模特互动融合（MODEL INTERACTION & HOME VIBE）：为了增添场景真实性并展现高雅家居美感，生图应该融入一位穿着柔软高弹力舒适睡衣或高质感象牙白家居服的女性模特，她正赤脚踩在厚实绵软的地毯表层缓慢行走、或者靠在沙发前舒适地坐在地毯上翻着看时尚杂志书册。她与地毯接触边缘要有极其逼真的受重压痕与阴影微凹陷，绝非漂浮拼贴。
4. 细节与风格（HIGH FIDELITY）：地毯图案与原图保持完全一致。
